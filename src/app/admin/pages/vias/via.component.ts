...
import { ViaFormComponent } from './via-form/via-form.component';
...
export class ViaComponent implements OnInit {
    ...
    ngOnInit() {
        this.viaService.getViaChange().subscribe((data) => {
            this.getPage(
                this.currentPage,
                this.rows,
                this.sortField,
        ...
        this.viaService.getMessageChange().subscribe((data) => {
    ...
    getPage(
        ...
        this.viaService
            ...
            .subscribe((res) => {
                this.vias = res.content;
                this.totalRecords = res.totalElements;
                this.loading = false;
            });
    }
    new() {
        this.ref = this.dialogService.open(ViaFormComponent, {
            header: 'Nueva Vía',
            width: '550px',
    ...
    edit(via: Via) {
        this.viaService.findById(via.idVia).subscribe((data) => {
            this.ref = this.dialogService.open(ViaFormComponent, {
                header: 'Editar Vía',
                width: '550px',
                contentStyle: { overflow: 'auto' },
    ...
    confirmDelete() {
        ...
        this.viaService.delete(this.via.idVia).subscribe((resp) => {
            this.messageService.add({
                severity: 'success',
                summary: 'Successful',
                detail: 'Vía eliminado con éxito',
                life: 3000,
            });
            this.via = new Via();
            this.getPage(
