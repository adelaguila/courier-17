...
export class PagoComponent implements OnInit {
    ...
    ngOnInit() {
        this.pagoService.getPagoChange().subscribe((data) => {
            this.getPage(
                this.currentPage,
                this.rows,
                this.sortField,
        ...
        this.cols = [
            { field: 'idPago', header: 'ID' },
            { field: 'fechaPago', header: 'Nombre' },
        ];
    ...
    new() {
        this.ref = this.dialogService.open(PagoFormComponent, {
            header: 'Nuevo Pago',
            width: '550px',
    ...
    edit(pago: Pago) {
        this.pagoService.findById(pago.idPago).subscribe((data) => {
            this.ref = this.dialogService.open(PagoFormComponent, {
                header: 'Editar Pago',
                width: '550px',
                contentStyle: { overflow: 'auto' },
    ...
    delete(pago: Pago) {
        this.deletePagoDialog = true;
        this.pago = { ...pago };
    }

    confirmDelete() {
        this.deletePagoDialog = false;
        this.pagoService.delete(this.pago.idPago).subscribe((resp) => {
            this.messageService.add({
                severity: 'success',
                summary: 'Successful',
                detail: 'Pago eliminado con éxito',
                life: 3000,
            });
            this.pago = new Pago();
            this.getPage(
